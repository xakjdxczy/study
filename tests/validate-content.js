...
ok(eggyHtml.includes("moveZone"), "eggy has a left-hand move zone");
ok(eggyHtml.includes("stickKnob"), "eggy has a virtual stick knob");
ok(eggyHtml.includes("btnJump"), "eggy has a right-hand jump button");
ok(eggyHtml.includes("btnDash"), "eggy has a right-hand dash button");
ok(eggyHtml.includes("左手移动"), "eggy tells players the left hand moves");
ok(eggyHtml.includes("右手"), "eggy tells players the right hand jumps");
ok(eggyHtml.includes("btnLobbyFs"), "eggy lobby has a fullscreen button");
ok(eggyHtml.includes("fullBtn"), "eggy race HUD has a fullscreen button");
...
ok(eggyJs.includes("requestFullscreen") || eggyJs.includes("webkitRequestFullscreen"), "eggy can request browser fullscreen");
ok(eggyJs.includes("webkitfullscreenchange"), "eggy listens for iOS/webkit fullscreen");
ok(eggyJs.includes("enterFs"), "eggy can enter fullscreen from a start tap");
ok(eggyJs.includes("bindHands"), "eggy binds split-hand controls");
ok(eggyJs.includes("setPointerCapture"), "eggy tracks multi-touch with pointer capture");
ok(!eggyJs.includes("pointerleave"), "eggy no longer drops keys on pointerleave");
ok(eggyJs.includes("function sy"), "eggy maps world Y onto the visible playfield");
ok(eggyJs.includes("keys.r = 1"), "holding the left stick runs forward");
ok(eggyJs.includes("jHold"), "eggy latches a tap jump so the server can see it");
ok(eggyHtml.includes("按住左手就会往前跑"), "eggy tells players that holding left runs forward");
